        ...
        <div className="h-[calc(100vh-100px)] flex flex-col animate-fade-in">
            ...
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-4">
                    <button onClick={() => navigate(-1)} className="p-2 hover:bg-white/10 rounded-full text-white/60 hover:text-white transition-colors">
                        <ArrowLeft size={20} />
                    </button>
                    <div>
                        <div className="flex items-center gap-3">
                            <h1 className="text-2xl font-bold text-white tracking-wide">CASE #{claim.refId || '???'}</h1>
                        ...
                        <p className="text-white/40 text-xs font-mono mt-1">OPENED {dateStr}</p>
                    </div>
                </div>
            ...
            <div className="flex-1 grid grid-cols-12 gap-6 min-h-0">
                ...
                <div className="col-span-12 lg:col-span-7 flex flex-col gap-6 overflow-y-auto pr-2">
                    <GlassCard className="p-6 !bg-black/20 !border-white/5">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-col gap-1 mt-1">
                                         <div className="flex items-center gap-4">
                                            <span className="flex items-center gap-1 text-xs text-white/40"><Plane size={12}/> {claim.flightNumber || 'No Flight'}</span>
                                            <span className="flex items-center gap-1 text-xs text-white/40"><MapPin size={12}/> {claim.location || 'Unknown'}</span>
                                         </div>
                                         {claim.contactValue && (
                                            <span className="text-xs font-mono text-cyan-400 bg-cyan-500/10 px-2 py-0.5 rounded w-fit">
                        ...
                        <div className="bg-white/5 rounded-xl p-4 border border-white/5">
                            <p className="text-white/80 leading-relaxed text-sm">"{claim.desc}"</p>
                        </div>
                    </GlassCard>
                ...
                <div className="col-span-12 lg:col-span-5 flex flex-col gap-4">
                    <GlassCard className="!bg-gradient-to-b !from-indigo-900/40 !to-black/40 !border-indigo-500/20 p-5 relative overflow-hidden">
                        <div className="flex items-center gap-2 mb-4">
                            <Sparkles className="text-indigo-400" size={16} />
                            <h3 className="text-xs font-bold text-indigo-300 uppercase tracking-widest">AI Analysis</h3>
                        </div>
                        {aiAnalysis ? (
                            <div className="space-y-4">
                                <p className="text-sm text-white/80">{aiAnalysis.summary}</p> 
                                <span className="text-xs text-indigo-300">Sentiment: {aiAnalysis.sentiment}</span>
                            </div>
                        ) : <div className="text-white/30 text-xs">Analysis pending...</div>}
                    </GlassCard>

                    <GlassCard className="flex-1 flex flex-col p-5 !bg-black/30 !border-white/5">
                        ...
                        <textarea 
                            className="flex-1 bg-black/20 rounded-lg border border-white/5 p-4 text-sm text-white/80 resize-none"
                            value={reply}
